...
if __name__ == "__main__":
    ...
    imagels = sorted(glob.glob(os.path.join(path, '*depth.png')))
    n_imgs = len(imagels)
    prefixls = []
    for image in imagels:
        prefixls.append(image.split(".")[0][:-6])
    ...
    for prefix in prefixls:
        ...
        depth_im = cv2.imread(prefix + "_depth.png", -1).astype(float) / 1000.  # depth is saved in 16-bit PNG in millimeters
        depth_im[depth_im > 1.2] = 0  # set invalid depth to 0 (specific to 7-scenes dataset)
    ...
    tsdf_vol = TSDFVolume(vol_bnds, voxel_size=0.0025, work_space=np.array([0.5,1,-1,1,0,1]))
    ...
    for prefix in prefixls:
        ...
        color_image = cv2.cvtColor(cv2.imread(prefix + "_rgb.png"), cv2.COLOR_BGR2RGB)
